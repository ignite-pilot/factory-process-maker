...
import type { WorkUnitCreateRequest } from "../api/client"

interface WorkUnitCreatorProps {
  nextSequence: number
  ...
  onCreate: (body: WorkUnitCreateRequest) => void
  onCancel: () => void
}
...
export default function WorkUnitCreator({
  nextSequence,
  defaultStartTime,
  defaultEndTime,
  duration,
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (endTime !== defaultEndTime || startTime === time) {
      setStartTime(time)
      setEndTime(time)
      onSeek(time)
    }
    setIsDragging(true)
    const clampedEnd = Math.max(startTime, time)
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const rect = e.currentTarget.getBoundingClientRect()
    const time = Math.max(startTime, ratioToTime(e.clientX, rect))
    setEndTime(time)
    onSeek(time)
  }
  ...
  return (
    <div className="mt-3 bg-slate-800 border border-emerald-700 rounded-lg p-3">
      <div className="flex items-center justify-between mb-2">
        <p className="text-emerald-300 text-xs font-medium">+ 새 작업 단위 추가</p>
      ...
      >
        ...
        {endTime > startTime && (
          <div
            className="absolute top-0 h-full bg-emerald-500 opacity-20 pointer-events-none"
      ...
      <div className="mb-2">
        ...
        <input
          ...
          className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500"
      ...
      <div className="mb-2">
        ...
        <textarea
          ...
          rows={2}
          className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 resize-none"
          placeholder="작업 설명"
        />
      </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          disabled={!title.trim()}
          className="flex-1 bg-emerald-600 text-white text-sm py-1.5 rounded hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          + 작업 추가
        </button>
        <button
